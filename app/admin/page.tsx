  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: assessments, error: aError } = await supabase
        .from("assessments")
        .select("id, user_id, vehicle_id, city, address, state, pincode, country, status, created_at, payment_required, payment_status, payment_amount")
      ...
      setRows(base.map((x: any) => ({
        ...
        payment_required: x.payment_required,
  ...
  return <main>
    ...
    <section className="section"><div className="container">
      ...
      <div style={{display:"grid",gap:12,marginTop:20}}>{visible.map(r=><div key={r.id} className="assessment-card">
        ...
        <div className="admin-assessment-meta"><div><small>CUSTOMER VEHICLE</small><strong>{r.vehicle?.make||"Vehicle"} {r.vehicle?.model||""}</strong></div><div><small>LOCATION</small><strong>{r.city||"Not provided"}</strong></div><div><small>AI SEVERITY</small><strong>{r.severity||"—"}</strong></div><div><small>PAYMENT</small><strong>{r.payment_required ? (r.payment_status === "paid" ? "Paid · ₹199" : `Pending · ₹${Number(r.payment_amount || 199).toLocaleString("en-IN")}`) : "Free"}</strong></div><div><small>ESTIMATE</small><strong>{r.minCost!=null&&r.maxCost!=null?`₹${Number(r.minCost).toLocaleString("en-IN")} – ₹${Number(r.maxCost).toLocaleString("en-IN")}`:"—"}</strong></div></div>
