...
export default function AdminCustomersPage() {
  ...
  useEffect(()=>{(async()=>{
    ...
    (assessments??[]).forEach((a:any)=>{if(a.user_id)counts.set(a.user_id,(counts.get(a.user_id)??0)+1);});
    setCustomers((profiles??[]).map((p:any)=>({...p,assessmentCount:counts.get(p.id)??0})));
    setLoading(false);
  ...
  return <main>
    ...
    <section className="section"><div className="container">
      <p className="muted">Administration</p><h1>👥 Customers</h1>
      <p className="muted">Select a customer to manage their assessments and manual assessment history.</p>
      {error&&<div className="card" style={{marginTop:20}}><p>{error}</p></div>}

      <div className="card" style={{marginTop:24}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}>
          <div><h2>Customer accounts</h2><p className="muted" style={{marginTop:4}}>{customers.length} registered customer{customers.length===1?"":"s"}</p></div>
          <input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Search name, phone or email" style={{padding:12,borderRadius:8,border:"1px solid #ccc",minWidth:280}} />
        </div>

        <div style={{display:"grid",gap:12,marginTop:18}}>
          {visible.map(customer=><a key={customer.id} href={`/admin/customers/${customer.id}`} className="card" style={{padding:16,textDecoration:"none",color:"inherit"}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <strong>{customer.full_name||"Customer"}</strong>
              <span className="muted">Joined {new Date(customer.created_at).toLocaleDateString("en-IN")}</span>
            </div>
            ...
            <p className="muted" style={{marginTop:10}}>Open customer profile →</p>
          </a>)}
