...
export default function AdminCustomersPage() {
  ...
  if (loading) {
    return <main className="section"><div className="container">Loading customers...</div></main>;
  }
    ...
    <main>
      ...
      <section className="section">
        <div className="container">
          ...
          <h1>👥 Customers</h1>
          <p className="muted">View all registered customers, including customers who have not created an assessment yet.</p>
          ...
          <div className="card" style={{marginTop:24}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}>
              <div>
                ...
                <p className="muted" style={{marginTop:4}}>{customers.length} registered customer{customers.length === 1 ? "" : "s"}</p>
              </div>
              <input
                value={filter}
                onChange={(e)=>setFilter(e.target.value)}
                placeholder="Search name, phone or email"
                style={{padding:12,borderRadius:8,border:"1px solid #ccc",minWidth:280}}
              />
            </div>
