...
export default function ManualAssessmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [saving,setSaving]=useState(false); const [error,setError]=useState(""); const [customerId,setCustomerId]=useState("");

  useEffect(()=>{(async()=>{ const {id}=await params; setCustomerId(id); const {data:{user}}=await supabase.auth.getUser(); if(!user){window.location.href="/login";return;}
