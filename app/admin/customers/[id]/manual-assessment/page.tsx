...
export default function ManualAssessmentPage({ params }: { params: { id: string } }) {
  ...
  useEffect(()=>{(async()=>{ const {data:{user}}=await supabase.auth.getUser(); if(!user){window.location.href="/login";return;}
    ...
    const [{data:c},{data:v}]=await Promise.all([supabase.from("profiles").select("id,full_name,phone,email").eq("id",params.id).eq("role","customer").single(),supabase.from("vehicles").select("id,make,model,year").eq("user_id",params.id).order("created_at",{ascending:false})]);
    if(!c){setError("Customer not found");return;} setCustomer(c);setVehicles(v??[]);if(v?.[0])setVehicleId(v[0].id);
  })().catch(e=>setError(e.message));},[]);

  async function save(){ setSaving(true);setError(""); const {data:{user}}=await supabase.auth.getUser(); if(!user){setError("Admin session expired");setSaving(false);return;}
    const {data,error}=await supabase.from("manual_assessments").insert({customer_id:params.id,vehicle_id:vehicleId||null,damage_description:damage,damaged_parts:parts.split("\n").map(x=>x.trim()).filter(Boolean),recommendations,repair_or_replacement:repair,estimated_min_cost:minCost?Number(minCost):null,estimated_max_cost:maxCost?Number(maxCost):null,estimated_time:time,shop_name:shopName,shop_phone:shopPhone,shop_address:shopAddress,shop_services:services.split(",").map(x=>x.trim()).filter(Boolean),additional_notes:notes,created_by:user.id}).select("id").single();
    if(error){setError(error.message);setSaving(false);return;} window.location.href=`/admin/customers/${params.id}/manual-assessment/${data.id}`;
  }
  ...
  return <main><header className="nav"><div className="container" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div className="brand"><span>Car</span>Fix <small style={{fontSize:13,marginLeft:8}}>Admin</small></div><a className="btn" href="/admin/customers">← Customers</a></div></header><section className="section"><div className="container"><p className="muted">Manual assessment</p><h1>Create manual assessment</h1><div className="card" style={{marginTop:20}}><strong>{customer.full_name||"Customer"}</strong><p className="muted">{customer.phone||"No phone"} · {customer.email||"No email"}</p></div>{error&&<div className="card" style={{marginTop:16}}><p>{error}</p></div>}
