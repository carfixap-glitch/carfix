...
export default function ManualAssessmentPage() {
  ...
  })().catch(e=>setError(e instanceof Error?e.message:"Could not load customer"));},[customerId]);

  async function save(){ setSaving(true);setError(""); const {data:{user}}=await supabase.auth.getUser(); if(!user){setError("Admin session expired");setSaving(false);return;} if(!customerId || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(customerId)){setError("Invalid customer ID. Please reopen this assessment from Assessment Management.");setSaving(false);return;}
    const {data,error}=await supabase.from("manual_assessments").insert({customer_id:customerId,vehicle_id:vehicleId||null,damage_description:damage,damaged_parts:parts.split("\n").map(x=>x.trim()).filter(Boolean),recommendations,repair_or_replacement:repair,estimated_min_cost:minCost?Number(minCost):null,estimated_max_cost:maxCost?Number(maxCost):null,estimated_time:time,shop_name:shopName,shop_phone:shopPhone,shop_address:shopAddress,shop_services:services.split(",").map(x=>x.trim()).filter(Boolean),additional_notes:notes,created_by:user.id}).select("id").single();
    if(error){setError(error.message);setSaving(false);return;} window.location.href=`/admin/customers/${customerId}/manual-assessment/${data.id}`;
  }
